...
var createSecurityProvider = function(config) {
  ...
  // ignore requests for /favicon.ico and any stylesheets
  provider.addIgnore(function(req, res) {
    return (req.method == 'GET' && /favicon.ico|.css/.test(req.url));
  });

  // ignore anything located in a root /public/ directory
  provider.addIgnore(function(req, res) {
    return (/^\/public/.test(req.url));
  });

  provider.define('loginShow', function(req, res) {
    return (req.method == 'GET' && /^\/login/.test(req.url));
  });

  provider.define('loginRequest', function(req, res) {
    return (req.method == 'POST' && /^\/login/.test(req.url));
  });

  provider.define('authenticate', function(req, res) {
    return (!/^\/login/.test(req.url) && !/^\/logout/.test(req.url));
  });
